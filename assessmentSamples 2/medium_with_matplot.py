...
class DataProcessor : 
    ...
    def estimateMark (self, student, task) :
        ...
        for entry in markRanks :
            if counter != student and entry[0] != -1 : # So that we omit the student that we are estimating
                rank = [entry[1]]
...
class UI : 
    ...
    def printEstimate (self, student, task, markEstimate, rankEstimate, markbook) :
        ...
        marksForTask = markbook.getTaskMarks(task)
        ...
        students = list(range(1,len(marksForTask) + 1))
        ...
        b, a = numpy.polyfit(x, y, deg=1)
        xseq = numpy.linspace(0, len(marksForTask) + 1, num=100)
